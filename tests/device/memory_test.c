...
int main(void) {
  ...
  const uint32_t bit_alias = 0x42000000u + 1u * 32u + 3u * 4u;
  TEST_EXPECT(&state,
              cortex_m4_write_memory(kinetis_k22_cpu(device), bit_alias, 4, 1));
  uint8_t byte = 0;
  TEST_EXPECT(&state, kinetis_k22_read(device, 0x40000001u, &byte, 1));
  TEST_EXPECT(&state, byte == 8u);
  TEST_EXPECT(&state, cortex_m4_read_memory(kinetis_k22_cpu(device), bit_alias,
                                            4, &value));
