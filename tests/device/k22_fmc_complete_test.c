...
static uint8_t read_flash(TestState* state, KinetisK22* device, uint32_t address,
    ...
    uint32_t value = UINT32_MAX;
    expect(state, kinetis_k22_memory_read(device, address, 1u, access, &value),
           "kinetis_k22_memory_read(device, address, 1u, access, &value)");
...
static void test_lru_and_lock(TestState* state) {
    ...
    expect(state, kinetis_k22_flash_controller_write(device, addresses[1], 1u, 0x22u),
           "kinetis_k22_flash_controller_write(device, addresses[1], 1u, 0x22u)");
    ...
    expect(state, kinetis_k22_flash_controller_write(device, addresses[0], 1u, 0x32u),
           "kinetis_k22_flash_controller_write(device, addresses[0], 1u, 0x32u)");
...
static void test_bank_one_cache(TestState* state) {
    ...
    uint8_t data = 0x22u;
    expect(state, kinetis_k22_write(device, 0x100u, &program, sizeof(program)),
           "kinetis_k22_write(device, 0x100u, &program, sizeof(program))");
    expect(state, kinetis_k22_write(device, FLEXNVM + 0x100u, &data, sizeof(data)),
...
static void test_flash_collision_irq(TestState* state) {
    ...
    expect(state, cortex_m4_get_irq_pending(kinetis_k22_cpu(device), 19u),
           "cortex_m4_get_irq_pending(kinetis_k22_cpu(device), 19u)");
