...
int main(void) {
    ...
    write_value(&state, device, MCG_S, 1, 0x1cu);
    ...
    TEST_EXPECT(&state, read_value(&state, device, LPTMR0_CSR, 4) == 0x81u);
    ...
    TEST_EXPECT(&state, read_value(&state, device, SIM_SCGC5, 4) == 0xa55ac33cu);
