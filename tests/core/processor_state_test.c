...
static KinetisK22* create_device(TestState* state) {
    ...
    expect(state, kinetis_k22_load(device, 0x200, usage_fault, sizeof(usage_fault)),
...
static void load_program(TestState* state, KinetisK22* device, const uint16_t* program,
                         size_t size) {
    expect(state, kinetis_k22_load(device, 0x100, program, size),
           "kinetis_k22_load(device, 0x100, program, size)");
...
int main(void) {
    ...
    expect(&state, kinetis_k22_load(device, 0x180, read_masks, sizeof(read_masks)),
