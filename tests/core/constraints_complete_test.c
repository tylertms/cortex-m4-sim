...
static void test_opcode_matrix(TestState* state, CortexM4* cpu) {
    const OpcodeConstraint cases[] = {
        ...
        {0xf011u, 0x0f01u, 0x0cu, true, CORTEX_M4_INSTRUCTION_UNDEFINED},
    ...
    for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); index++) {
        ...
        if (actual != cases[index].expected) {
            fprintf(stderr, "constraint case %zu: %04x %04x expected %u got %u\n",
                    index, cases[index].first, cases[index].second,
...
int main(void) {
    ...
    TEST_EXPECT(&state, cortex_m4_check_instruction_constraints(
                            NULL, 0, 0, false) ==
