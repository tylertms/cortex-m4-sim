...
static void execute(TestState* state, KinetisK22* device, uint16_t opcode) {
...
int main(void) {
    ...
    TEST_EXPECT(&state, kinetis_k22_write(device, address, &initial, sizeof(initial)));
    ...
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == initial);
    execute(&state, device, 0x5a88u);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == 0x7f01u);
    ...
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == 0xffffffffu);
    const int16_t negative_halfword = -128;
    TEST_EXPECT(&state, kinetis_k22_write(device, address + 4, &negative_halfword,
                                          sizeof(negative_halfword)));
    cortex_m4_set_register(cpu, 2, 4);
    execute(&state, device, 0x5e88u);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == 0xffffff80u);

    cortex_m4_set_register(cpu, 2, 4);
    cortex_m4_set_register(cpu, 0, 0x11223344u);
    execute(&state, device, 0x5088u);
    uint32_t stored = 0;
    TEST_EXPECT(&state, kinetis_k22_read(device, address + 4, &stored, sizeof(stored)));
    TEST_EXPECT(&state, stored == 0x11223344u);
    cortex_m4_set_register(cpu, 2, 6);
    cortex_m4_set_register(cpu, 0, 0xabcd1234u);
    execute(&state, device, 0x5288u);
    uint16_t halfword = 0;
    TEST_EXPECT(&state, kinetis_k22_read(device, address + 6, &halfword, sizeof(halfword)));
    TEST_EXPECT(&state, halfword == 0x1234u);
    cortex_m4_set_register(cpu, 2, 8);
    execute(&state, device, 0x5488u);
    uint8_t byte = 0;
    TEST_EXPECT(&state, kinetis_k22_read(device, address + 8, &byte, sizeof(byte)));
    TEST_EXPECT(&state, byte == 0x34u);

    cortex_m4_set_register(cpu, 1, address);
    cortex_m4_set_register(cpu, 0, 0x55667788u);
    execute(&state, device, 0x6048u);
    execute(&state, device, 0x684au);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 2) == 0x55667788u);
    execute(&state, device, 0x7048u);
    execute(&state, device, 0x784au);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 2) == 0x88u);
    execute(&state, device, 0x8048u);
    execute(&state, device, 0x884au);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 2) == 0x7788u);
    ...
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == 0x20000108u);
    ...
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 13) == original_stack - 8);
    cortex_m4_set_register(cpu, 0, 0);
    cortex_m4_set_register(cpu, 14, 0);
    execute(&state, device, 0xbc01u);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 0) == 0x12345678u);
    TEST_EXPECT(&state, cortex_m4_get_register(cpu, 13) == original_stack - 4);
