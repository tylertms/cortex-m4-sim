...
static uint8_t current_priority(const CortexM4* cpu) {
    ...
    if (exception < 16) {
        return exception >= 4 ? cpu->system_priority[exception - 4] : 0;
    }
...
static bool enter_exception(CortexM4* cpu, uint16_t exception) {
    ...
    const bool align_padding = (cpu->ccr & (1u << 9)) != 0 && (stack_pointer & 7u) != 0;
    if (align_padding) {
        stack_pointer -= 4;
    }
    stack_pointer -= 32;
...
bool cortex_m4_take_pending_exception(CortexM4* cpu) {
    ...
    for (uint8_t index = 0;
        ...
        const uint8_t priority = cpu->system_priority[exception - 4];
...
bool cortex_m4_exception_return(CortexM4* cpu, uint32_t value) {
    if (value != EXCEPTION_RETURN_HANDLER_MSP && value != EXCEPTION_RETURN_THREAD_MSP &&
