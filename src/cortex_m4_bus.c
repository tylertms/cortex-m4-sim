...
bool cortex_m4_bus_read(CortexM4* cpu, uint32_t address, uint8_t size,
    ...
    const CortexM4SystemAccess system_access =
        cortex_m4_system_read(cpu, address, size, value);
...
bool cortex_m4_bus_write(CortexM4* cpu, uint32_t address, uint8_t size,
    ...
    const CortexM4SystemAccess system_access =
        cortex_m4_system_write(cpu, address, size, value);
    if (system_access == CORTEX_M4_SYSTEM_ACCESS_ACCEPTED) {
        cortex_m4_timing_observe_write(cpu, address, size);
        return true;
    }
...
bool cortex_m4_require_alignment(CortexM4* cpu, uint32_t address,
                                 uint8_t alignment) {
